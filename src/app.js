...
function gerarDocumento() {
    console.log('gerarDocumento called');
    ...
    fetch('Procuracao modelo.docx')
        .then(response => {
            if (!response.ok) {
                throw new Error('Erro ao carregar o modelo de documento.');
            }
        ...
        .then(arrayBuffer => {
            ...
            try {
                ...
                document.getElementById('status').innerText = "Erro ao processar o documento.";
                return;
            }
            ...
            try {
                ...
                saveAs(out, `procuracao_de_${nome}.docx`);
                document.getElementById('status').innerText = "Documento gerado com sucesso!";
            } catch (error) {
                console.error('Erro ao renderizar o documento:', error);
                document.getElementById('status').innerText = "Erro ao gerar o documento.";
            }
        })
        .catch(error => {
            console.error('Erro no processamento do fetch:', error);
            document.getElementById('status').innerText = "Erro ao carregar o modelo de documento.";
        });
}
